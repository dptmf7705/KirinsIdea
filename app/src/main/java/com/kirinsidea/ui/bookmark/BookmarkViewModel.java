...
//           TODO. bookmark.getPath() 해당 경로 파일 읽어서 본문 반환
            return Html.fromHtml("<h1>메인 스레드와 Handler</h1><p style=\"text-align: left; clear: none; float: none;\"><br /></p><h2>1. UI 작업을 메인 스레드에서만 하는 이유</h2><p>안드로이드에서의 UI 작업을 담당하는 메인 스레드(UI 스레드)는&nbsp;싱글 스레드 모델을 원칙으로한다. 뷰나 뷰그룹이 여러 스레드에 의해 업데이트될 수 있다면 교착 상태(Deadlock)나 경합상태(Race Condition) 등 여러문제가 발생할 수 있다.&nbsp;</p>\n" +
                    "<p><b><br /></b></p>\n" +
    ...
    @NonNull
    public LiveData<Spanned> getContent() {
        return content;
    }
}
